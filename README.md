...
Codex Flow 是一款面向 Windows PC 的多模型 AI 客户端，目标是让用户只需填写 API Key，就能立即使用真实模型对话。项目同时提供图形桌面端和终端端，两端共享加密配置、当前模型与 Token 使用记录。
...
1. Electron 桌面窗口、无窗口终端应用和随客户端安装的官方 Codex Agent 运行时。
...
API Key 只保存为系统加密后的密文。`logout` 会清除服务商和 API Key 配置，但保留历史 Token 用量。
