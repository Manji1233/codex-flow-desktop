...
ChatGPT Codex 桌面客户端把 `openai/codex` 官方运行时装进一个安装包。默认使用 **API Key** 模式，选择平台、粘贴密钥、点击发送，即可调用真实大模型；也支持一键切换到 ChatGPT 账户模式，退出后本地 API Key 仍然保留。

## 为什么容易上手

| 步骤 | 操作 | 耗时 |
|------|------|------|
| 1. 下载安装 | 从 [Releases](https://github.com/Manji1233/codex-flow-desktop/releases/latest) 下载 `ChatGPT-Codex-Setup-*.exe` | 1 分钟 |
| 2. 填写 API Key | 打开软件 → 选择厂商（OpenAI / 火山方舟 / 阿里云 / 智谱 / 腾讯 / 自定义）→ 粘贴密钥 | 30 秒 |
| 3. 开始对话 | 输入问题，自动联网搜索、自动选择模型、自动统计 Token | 立刻 |

不需要 Docker、不需要 Python 环境、不需要手动安装 Codex CLI，安装包里已经内置 `codex-cli` 和常用官方插件。

## 装好就能用的能力

- **真实对话**：流式输出、图片附件、多轮上下文、自动联网搜索并标注来源。
- **代码助手**：启动 Codex Agent，直接修改文件、运行终端、审查代码 Diff、执行多步骤任务。
- **视频剪辑**：首次打开自动准备 Remotion、HyperFrames 和离线 `$video-studio`，选素材即可生成视频。
- **定时任务**：设置每天/每周/每小时执行提示词，客户端运行时会自动调用 Agent。
- **Token 账本**：每次请求自动记录，今日、本月、今年用量一目了然。
- **桌面 + 终端**：图形界面和 `codex-flow` 命令行共用同一套配置与账本。
...
- 客户端更新：安装版通过 GitHub Release 检查、下载并安装新版本，支持下载进度和更新说明。
- 历史保护：每次版本变化和安装更新前，自动在本机备份 API 配置、Token 账本、定时任务、提示词与会话历史；可从更新中心恢复并重启。
...
- 热门视频 Skills：首次启动自动安装 Codex 官方精选 Remotion 与 HyperFrames，并始终提供离线 `$video-studio` 编排 Skill；网络异常时仍可回退到本机 FFmpeg 工作流。
- 自定义 MCP：支持添加远程 URL 或本地 stdio MCP 服务。
- Codex 风格排版：支持标题、列表、表格、代码块、引用、链接和 Markdown 导出。
- 桌面与终端共享：两种使用方式读取同一份安全配置和用量账本。
- 视频工作室：选择本地素材后由 `$video-studio` 自动选择 Remotion、HyperFrames 或 FFmpeg，必须渲染并验证真实可播放视频文件。
...
- API Key、Token 记录、任务和历史数据保存在 `%APPDATA%\codex-flow-desktop`，不写入安装目录，因此覆盖安装不会删除历史。
- 自动备份保存在 `%APPDATA%\codex-flow-desktop\backups`，默认保留最近 12 份。
- 安装更新前会再次创建完整本地备份；备份失败时“备份并安装”按钮会被禁用。
- 安装或恢复备份时会先停止当前 Agent、终端和本地调度，避免更新过程中继续写入历史文件。
- NSIS 卸载器配置为不删除用户数据。用户只有手动删除上述用户数据目录时才会失去历史记录。
- 开发模式只展示更新功能，不执行自动安装；GitHub Release 安装版才启用真实更新。
- 安装版先通过 GitHub API 获取最新版本，再直接读取对应标签目录中的 `latest.yml`，不依赖容易受网络影响的 Releases Atom Feed 或 Latest 跳转地址。
...
发布正式版本时，先更新 `package.json` 版本并提交，然后运行：

```powershell
npm.cmd run release
```

`scripts/publish-release.ps1` 会自动检查、测试、构建 Windows x64 安装包，并创建或更新对应版本的 GitHub Release，上传安装程序、`latest.yml` 和差分更新文件。它只需要 GitHub `repo` 权限，不要求额外的 `workflow` 权限。后续版本重复提升版本号、提交并运行该命令即可。
